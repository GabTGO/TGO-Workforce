"""App-wide settings a Super Admin controls at runtime — currently just
whether sign-in is locked to invited emails only. See
...
router = APIRouter(prefix="/app-settings", tags=["app-settings"], dependencies=[Depends(require_super_admin)])
...
async def update_settings(payload: AppSettingsUpdate, db: DbSession, account: SuperAdmin) -> AppSettingsRead:
    ...
    if changes:
        await record_activity(
            db,
            action="Updated sign-in access settings",
